...
func KeychainResourceSchema(ctx context.Context) schema.Schema {
		...
		Attributes: map[string]schema.Attribute{
			...
			"api_version": schema.StringAttribute{
				...
				Validators: []validator.String{
					stringvalidator.RegexMatches(regexp.MustCompile("^security\\.eda\\.nokia\\.com/v1alpha1$"), ""),
				},
				Default: stringdefault.StaticString("security.eda.nokia.com/v1alpha1"),
			},
			...
			"kind": schema.StringAttribute{
				...
				Computed: true,
				Validators: []validator.String{
					stringvalidator.RegexMatches(regexp.MustCompile("^Keychain$"), ""),
				},
				Default: stringdefault.StaticString("Keychain"),
			},
			...
			"spec": schema.SingleNestedAttribute{
				...
				MarkdownDescription: "KeychainSpec defines the desired state of Keychain",
			},
			"status": schema.SingleNestedAttribute{
				...
				MarkdownDescription: "KeychainStatus defines the observed state of Keychain",
			},
		},
	}
}

type KeychainModel struct {
...
func (t KeyType) ValueFromObject(ctx context.Context, in basetypes.ObjectValue) (basetypes.ObjectValuable, diag.Diagnostics) {
	...
	authenticationKeyAttribute, ok := attributes["authentication_key"]

	if !ok {
		diags.AddError(
			"Attribute Missing",
			`authentication_key is missing from object`)

		return nil, diags
	}

	authenticationKeyVal, ok := authenticationKeyAttribute.(basetypes.StringValue)

	if !ok {
		diags.AddError(
			"Attribute Wrong Type",
			fmt.Sprintf(`authentication_key expected to be basetypes.StringValue, was: %T`, authenticationKeyAttribute))
	}
	...
	return KeyValue{
		...
		AuthenticationKey: authenticationKeyVal,
		state:             attr.ValueStateKnown,
	}, diags
}
...
func (v KeyValue) ToTerraformValue(ctx context.Context) (tftypes.Value, error) {
	...
	attrTypes["authentication_key"] = basetypes.StringType{}.TerraformType(ctx)
	...
	case attr.ValueStateKnown:
		...
		val, err = v.AuthenticationKey.ToTerraformValue(ctx)

		if err != nil {
			return tftypes.NewValue(objectType, tftypes.UnknownValue), err
		}

		vals["authentication_key"] = val
...
func (v KeyValue) ToObjectValue(ctx context.Context) (basetypes.ObjectValue, diag.Diagnostics) {
	...
	objVal, diags := types.ObjectValue(
		...
		map[string]attr.Value{
			...
			"authentication_key": v.AuthenticationKey,
		})
...
func (v KeyValue) Equal(o attr.Value) bool {
	...
	if !v.AuthenticationKey.Equal(other.AuthenticationKey) {
		return false
	}
